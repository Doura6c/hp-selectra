...
export async function generateStaticParams() {
  return PROVIDERS.filter((p) => p.verticalSlug === "banques").map((p) => ({ slug: p.slug }))
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params
  const provider = PROVIDERS.find((p) => p.slug === slug && p.verticalSlug === "banques")
  if (!provider) return {}
  return {
    title: `${provider.name} — Comptes, cartes et avis | HP Selectra Guinée`,
    description: provider.description,
  }
}

export default async function BanqueProviderPage({ params }: Props) {
  const { slug } = await params
  const provider = PROVIDERS.find((p) => p.slug === slug && p.verticalSlug === "banques")
  ...
  return (
    <>
      ...
      <div className="container py-4">
        <ProviderMeta verticalSlug="banques" updatedAt={new Date()} verified={provider.verified} />
      </div>
              ...
              <table className="w-full text-sm" style={{ backgroundColor: "var(--color-card)" }}>
                <thead>
                  <tr style={{ backgroundColor: "var(--color-primary)", color: "#fff" }}>
                    ...
                    <th className="text-left px-4 py-3 font-semibold">Frais indicatifs</th>
