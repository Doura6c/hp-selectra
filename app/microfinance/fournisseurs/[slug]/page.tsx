...
export async function generateStaticParams() {
  return PROVIDERS.filter((p) => p.verticalSlug === "microfinance").map((p) => ({ slug: p.slug }))
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params
  const provider = PROVIDERS.find((p) => p.slug === slug && p.verticalSlug === "microfinance")
  if (!provider) return {}
  return {
...
export default async function MicrofinanceProviderPage({ params }: Props) {
  const { slug } = await params
  const provider = PROVIDERS.find((p) => p.slug === slug && p.verticalSlug === "microfinance")
  if (!provider) notFound()

  const offers = OFFERS.filter((o) => o.providerSlug === slug && o.verticalSlug === "microfinance")

  const jsonLdBreadcrumb = buildJsonLd(breadcrumbSchema([
    { name: "Accueil", href: "/" },
    { name: "Microfinance", href: "/microfinance/" },
    { name: "Institutions", href: "/microfinance/fournisseurs/" },
    { name: provider.name },
  ]))
  const jsonLdRating = buildJsonLd(providerRatingSchema({
    name: provider.name,
    description: provider.description,
    url: `/microfinance/fournisseurs/${slug}/`,
  ...
  return (
    <>
      ...
      <section className="py-10 sm:py-14"
        style={{ background: `linear-gradient(135deg, ${provider.brandColor ?? "#4E6A2C"}cc, ${provider.brandColor ?? "#6B8F3C"})` }}>
        <div className="container">
          <Breadcrumb items={[
            { label: "Accueil", href: "/" },
            { label: "Microfinance", href: "/microfinance/" },
            { label: "Institutions", href: "/microfinance/fournisseurs/" },
            { label: provider.name },
          ]} />
      ...
      <div className="container py-10 max-w-4xl">
        ...
        <div className="grid lg:grid-cols-3 gap-6 mb-10">
          ...
          <div className="space-y-4">
            <ProviderMeta verticalSlug="microfinance" updatedAt={new Date()} verified={provider.verified} />
            ...
            <a href={`https://wa.me/${WA_NUMBER}?text=${encodeURIComponent(`Bonjour, je souhaite un devis ${provider.name} pour un micro-crédit en Guinée.`)}`}
