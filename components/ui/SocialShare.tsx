...
// Inline LinkedIn SVG since lucide-react doesn't export it
...
export default function SocialShare({ url, title, className = "" }: Props) {
  const [copied, setCopied] = useState(false)

  const pageUrl = url ?? (typeof window !== "undefined" ? window.location.href : "")
  const pageTitle = title ?? (typeof document !== "undefined" ? document.title : "HP Selectra Guinée")
  ...
  return (
    <div className={`flex flex-wrap items-center gap-2 ${className}`}>
      <span className="text-xs font-semibold mr-1" style={{ color: "var(--color-muted)" }}>
        Partager :
      </span>
      ...
      <button
        ...
        style={{
          borderColor: copied ? "#6B8F3C" : "var(--color-border)",
          color: copied ? "#6B8F3C" : "var(--color-text)",
          backgroundColor: copied ? "#e8f5e9" : "var(--color-card)",
        }}
      >
