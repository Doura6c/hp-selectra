...
export default function WhatsAppCTA() {
  ...
  return (
    <a
      ...
      aria-label="Parler à un conseiller Help'me Process sur WhatsApp"
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
      className="fixed bottom-5 right-5 z-50 flex items-center gap-3 shadow-xl transition-all duration-300"
      style={{
        backgroundColor: "var(--color-whatsapp)",
        borderRadius: hovered ? "1rem" : "9999px",
        padding: hovered ? "0.75rem 1.25rem" : "1rem",
      }}
    >
      {/* Icône WhatsApp */}
